    ...
    private Token NextToken()
    {
        ...
        switch (Current)
        {
            ...
            case ')':
                ...
                break;
            case '{':
                _tokenKind = TokenKind.OpenBraceToken;
                Advance();
                break;
            case '}':
            ...
            case '>':
                ...
                if (Current == '=')
                {
                    _tokenKind = TokenKind.GreaterThanOrEqualToken;
                    Advance();
                }
                else
                {
                    _tokenKind = TokenKind.GreaterThanToken;
                }
            ...
            case '<':
                ...
                if (Current == '=')
                {
                    _tokenKind = TokenKind.LessThanOrEqualToken;
                    Advance();
                }
                else
                {
                    _tokenKind = TokenKind.LessThanToken;
                }
            ...
            case '|':
                ...
                if (Current is '|')
                {
                    _tokenKind = TokenKind.LogicalOrToken;
                    Advance();
                }
                else
                {
                    ErrorsBag.ReportIllegalCharacterError(prevOr, posStart, _position);
                }
            ...
            case '&':
                ...
                if (Current is '&')
                {
                    _tokenKind = TokenKind.LogicalAndToken;
                    Advance();
                }
                else
                {
                    ErrorsBag.ReportIllegalCharacterError(prevAnd, posStart, _position);
                }
